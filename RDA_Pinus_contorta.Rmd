...
This tutorial provides code and explanation associated with a review of the different applications of RDA in the field of landscape genomics written by Thibaut Capblancq & Brenna Forester (2021). Redundancy Analysis (RDA): a Swiss-army knife for landscape genomics. XXXX  

To reproduce the different results and figures of this tutorial you will need to download the following datasets:

* Information on individuals and populations: [Pine_TotemField_AllNaturalIndivResiduals_Jan20th2017.csv ](https://datadryad.org/stash/dataset/doi:10.5061/dryad.56j8vq8)
* Genetic matrix for 32,407 SNPs in genic regions: [Pine_AllNatural_GCandTotemIndivs_GWAS_SNPs_June8th2019.txt](https://datadryad.org/stash/dataset/doi:10.5061/dryad.56j8vq8)
* Genetic matrix of 3,934 SNPs in intergenic regions: [Pine_AllNatural_GCandTotemIndivs_ControlSNPs_June8th2019.txt](https://datadryad.org/stash/dataset/doi:10.5061/dryad.56j8vq8)
* Geographic coordinates of sampled source populations: [PlSeedlots.csv](https://datadryad.org/stash/dataset/doi:10.5061/dryad.56j8vq8)
* 27 Climatic rasters from the ClimateNA database (http://climatena.ca), projected in WGS84 coordinate projection (you can reproduce our way to do that using the code in ClimateNA_formatting.R).
* The values of the nine retainded climatic variables (MAR, EMT, MWMT, CMD, Tave_wt, DD_18, MAP, Eref, PAS) in 2012, 2013, 2014 and 2015 at the Vancouver common garden site (latitude = 49.253, longitude = -123.25). You can extract these values using the ClimateNA website (http://www.climatewna.com).  
<br>  
...
The genomic data used to search for adaptive loci consisted of 32,407 SNPs genotyped using the AdapTree project lodgepole pine 50K SNP array (Mahony et al. 2020). 

**Reminder:** The SNP dataset can be downloaded [here](https://datadryad.org/stash/dataset/doi:10.5061/dryad.56j8vq8).   
...
**Reminder:** The metadata can be downloaded [here](https://datadryad.org/stash/dataset/doi:10.5061/dryad.56j8vq8)
...
The final genetic matrix included allele frequencies for 281 populations (rows) and 29,941 loci (columns).  
<br>  

<h4>1.2. Climatic variables</h4>  

The values of 27 bioclimate variables were extracted for all 281 populations from the [ClimateNA database](http://www.climatena.ca). We had to re-project the rasters into the WGS84 reference coordinate system; see our R code ClimateNA_formating.R if you want/need to do the same.
...
**Reminder:** The PlSeedlots.csv can be downloaded [here](https://datadryad.org/stash/dataset/doi:10.5061/dryad.56j8vq8)  
...
Finally, we standardized the climatic variables to avoid discrepancy in mean and standard deviation among variables (e.g., precipitation in mm and temperature in °C). 
...
**Reminder:** This set of neutral loci can be downloaded [here](https://datadryad.org/stash/dataset/doi:10.5061/dryad.56j8vq8).  
...
## Species range
range <- readOGR("./Data/pinucon/pinucont.shp") # the shapefile can be found here: https://sites.ualberta.ca/~ahamann/data/rangemaps.html
...
We used a pRDA to identify candidate adaptive markers in the lodgepole pine data using population allele frequencies for the 29,941 loci as response and the 9 selected bioclimate variables as predictors. We followed the procedure described in Capblancq et al. (2018) where outliers are identified based on their extremeness along a distribution of Mahalanobis distances estimated between each locus and the center of the RDA space using a certain number of axes (K).
...
In this case, we can load the [rdadapt](XXX) function, described in Capblancq et al. (2018) and use it to conduct the genome scan.
...
**Note:** the rdadapt function returns both p-values and q-values, which means it is possible to use a FDR (False Discovery Rate) approach instead of a p-value threshold to identify outliers.
...
We identified 583 loci showing extreme association with the environment, reduced to a set of 183 unlinked outliers when retaining only the best hit for each genomic contig.

<br>  

<h4>4.2. Comparing outlier list to Mahony et al. (2020)</h4>  

Among the 183 best hit outliers identified using pRDA (see above), 126 were also identified by Mahony et al. (2020) using Bayenv2, and a third (326) of the 865 loci identified with Bayenv2 were identified in the larger set of 583 RDA candidates.
...
For all of the following RDA-based analyses we decided to retain only the 95 loci that were detected by both the partial RDA that accounted for population structure and by the simple RDA that did not account for population structure.

**Note:** Decisions about if and how to correct for population structure, how to set outlier thresholds, and how to finalize a set of candidate adaptive markers will depend upon the objectives of the study and the data available (if any) for validation. The approach presented here is not meant to be applied without modification; modeling parameters and other decisions should be determined based on the characteristics of the data and the goals of the study.

```{r, message=FALSE, results='hide', eval=TRUE}
common_outliers_RDA_top <- Reduce(intersect, list_outliers_RDA_top)
```
```{r, message=FALSE, warning=FALSE, eval=TRUE, echo=FALSE}
common_outliers_RDA_top
```
<br>  
...
The 95 putative adaptive loci were used as multivariate response in a new “adaptively enriched” RDA, using the 9 selected bioclimate variables as explanatory variables. 
...
An RDA biplot allows us to visualize the relationship between the 95 putative adaptive loci and the underlying climatic variables.  
...
We found that genomic offset better explained the decrease in height and growth rate than the basic climate transfer distance, which confirms the great potential of genomic offset for selecting source population for management or conservation plans. 
